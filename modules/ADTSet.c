...
struct set_node {
	...
	int left_num;				// Ο αριθμός των κόμβων που υπάρχουν αριστερά από τον κόμβο
...
static SetNode node_rotate_left(SetNode node) {
	...
	// Ενημέρωση του πλήθους των αριστερότερων κόμβων της καινούργιας ρίζας, δηλαδή του παλιού δεξιού παιδιού.(τα υπόλοιπα παραμένουν ως έχουν)
	right_node->left_num += node->left_num + 1;
...
static SetNode node_rotate_right(SetNode node) {
	...
	// Ενημέρωση του πλήθους των αριστερότερων κόμβων της παλιάς ρίζας.(τα υπόλοιπα παραμένουν ως έχουν)
	node->left_num -= (left_node->left_num + 1);
...
// Η συνάρτηση αυτή βρίσκει αναδρομικά τον κόμβο με θέση διάταξης pos, ανάλογα με το πόσοι κόμβοι υπάρχουν αριστερά από τον κάθε κόμβο που επισκέπτεται

static Pointer node_get_at(SetNode node, int pos) {
	int new_pos = pos - node->left_num;		// αφαιρώ από την ζητούμενη θέση το σύνολο των αριστερότερων κόμβων για να βρω τη διάταξη της θέσης μου στο δέντρο
	if (new_pos == 0)	// αν η θέση μου είναι ίση με το σύνολο των αριστερότερων κόμβων τότε ο ζητούμενος κόμβος είναι η ρίζα(αφού οι κόμβοι αρχίζουν από 0)
		return node->value;
	else if (new_pos < 0)	// η θέση μου βρίσκεται μέσα στο αριστερό υποδέντρο άρα πάω να ψάξω εκεί
		return node_get_at(node->left, pos);
	else {		// η θέση μου δεν βρίσκεται ούτε στη ρίζα όυτε στο αριστερό υποδέντρο οπότε πάω να ψάξω στο δεξί υποδέντρο με την καινούργια θέση προς αναζήτηση
		new_pos--;	// αφαιρούμε και τη ρίζα
		return node_get_at(node->right, new_pos);	// ψάξε 
	}
}
...
static SetNode node_insert(SetNode node, CompareFunc compare, Pointer value, bool* inserted, Pointer* old_value) {
	...
	if (compare_res == 0) {
		...
		*old_value = node->value;
		...
		// αν έγινε εισαγωγή κόμβου αριστερότερά του τότε αύξησε τον αριθμό των αριστερότερων κόμβων του
		if (*inserted)
			node->left_num++;
...
static SetNode node_remove(SetNode node, CompareFunc compare, Pointer value, bool* removed, Pointer* old_value) {
	...
	if (compare_res == 0) {
		...
		*old_value = node->value;
		...
		if (node->left == NULL) {
			...
			min_right->left_num = node->left_num;
	...
	if (compare_res < 0) {
		...
		// αν έγινε διαγραφή κόμβου αριστερότερά του τότε μείωσε τον αριθμό των αριστερότερων κόμβων του
		if (*removed)
			node->left_num--;
	}
	else
...
Pointer set_get_at(Set set, int pos) {
	return node_get_at(set->root, pos);
}

void set_set_at(Set set, int pos, Pointer value) {
	Pointer node_value = node_get_at(set->root, pos);
	if (node_value == value)
		return;

	if (set->compare(node_value, value) == 0 && node_value != value) {
		if (set->destroy_value != NULL)
			set->destroy_value(node_value);
		node_value = value;
	}
	else {
		set_remove(set, node_value);		// αφαιρούμε από το set την παλιά τιμή
		set_insert(set, value);				// προσθέτουμε στο set τη νέα τιμή
	}
}
